...
export default function LiveChatPage() {
  ...
  // Filtra os chats para mostrar apenas os que pertencem ao admin logado.
  const myActiveChats = activeChatsData.filter(chat => chat.adminId === loggedInUserId);
  
  const [selectedChat, setSelectedChat] = useState<ActiveChat | null>(myActiveChats[0] || null);
    ...
    <div className="grid grid-cols-1 md:grid-cols-[350px_1fr] h-[calc(100vh-theme(spacing.16)-theme(spacing.16))] border rounded-lg overflow-hidden">
      ...
      <aside className="flex flex-col border-r bg-card">
        <header className="p-4 border-b">
          <h2 className="text-xl font-headline font-semibold">Minhas Conversas</h2>
          <p className="text-sm text-muted-foreground">Monitorando seus chats</p>
        </header>
            ...
            <header className="flex items-center justify-between p-4 border-b">
              ...
              <div className="flex items-center gap-2">
                 <Button variant="outline" size="sm" className="flex items-center gap-2">
                    <AlertCircle className="w-4 h-4 text-destructive" /> Encerrar
                 </Button>
              </div>
            </header>
