...
// --- PASSO 1: ATUALIZAR TIPO DO ESTADO ---
type AdminProfile = {
  name: string;
  avatarUrl?: string;
  headline?: string; // Adicionado
};
...
export function PublicChatView({ adminUid }: PublicChatViewProps) {
  ...
  // --- PASSO 2: ATUALIZAR BUSCA DE DADOS ---
  useEffect(() => {
    if (!adminUid) return;
      ...
      try {
        ...
        if (profileDocSnap.exists()) {
          ...
          setAdminProfile({
            ...
            headline: profileData.headline, // Adicionado
          });
        } else {
          console.warn(`Perfil público não encontrado para o admin: ${adminUid}. Usando fallback.`);
  ...
  const handleLeadCaptureSuccess = async (newContact: Contact) => {
    if (!adminUid || !conversationPath || !conversationPath.startsWith('conversations/anonymous_')) {
        toast({ title: 'Erro de Migração', description: 'Não foi possível encontrar a conversa anônima.', variant: 'destructive' });
        return;
    }
    ...
    try {
      ...
      toast({ title: 'Erro Crítico', description: 'Falha ao transferir seu histórico.', variant: 'destructive' });
    }
  };
  ...
  if (isChatClosed) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center bg-background text-center p-8">
        ...
        <h1 className="text-2xl font-bold text-foreground mb-2">Obrigado pelo seu contato!</h1>
        <p className="text-lg text-muted-foreground">Agradecemos a sua preferência.</p>
  ...
  return (
    <>
      <div className="flex h-screen flex-col bg-background">
        ...
        <header className="flex items-center justify-between border-b p-4 shadow-sm bg-card">
          <div className="flex items-center gap-3">
            ...
            <div>
                <h1 className="text-lg font-headline font-semibold">{isAdminProfileLoading ? 'Carregando...' : adminProfile?.name || 'Assistente Virtual'}</h1>
                {/* --- PASSO 3: EXIBIR FRASE --- */}
                <p className="text-sm text-muted-foreground">{adminProfile?.headline || (isIdentified ? `Conectado com nosso time` : "Pronto para ajudar")}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" onClick={() => setShowCloseConfirmModal(true)} aria-label="Encerrar Chat">
        ...
        <ScrollArea className="flex-1 p-4" ref={scrollAreaRef}><div className="space-y-6 max-w-4xl mx-auto">
          {messages.map((message, index) => (
            <div key={index} className={cn("flex items-start gap-4", message.role === "user" ? "justify-end" : "justify-start")}>
              ...
              {message.role === "user" && (<Avatar className="h-8 w-8"><AvatarFallback>V</AvatarFallback></Avatar>)}
            </div>
          ))}
      ...
      <AlertDialog open={showCloseConfirmModal} onOpenChange={setShowCloseConfirmModal}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Deseja Mesmo Sair do Chat?</AlertDialogTitle>
            <AlertDialogDescription>
              Ao confirmar, esta conversa será encerrada.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={confirmCloseChat}>Sim, Sair</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
